...
try {
    ...
    // Check if we're requesting all campuses
    $all_campuses = isset($_GET['all_campuses']) && $_GET['all_campuses'] == 1;
    
    // Log for debugging
    error_log("Processing request for campus: " . $campus . ", all_campuses: " . ($all_campuses ? "true" : "false"));

    // First, check if we have any records
    if ($all_campuses) {
        // Don't filter by campus if all_campuses=1
        $checkQuery = "SELECT COUNT(*) as count FROM ppas_forms WHERE 1";
    ...
    error_log("Found {$recordCount} records " . ($all_campuses ? "across all campuses" : "for campus {$campus}"));

    // Modified query based on all_campuses flag
    if ($all_campuses) {
        // For all campuses, don't filter by campus
