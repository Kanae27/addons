...
try {
    ...
    $is_all_campus = ($campus === 'All Campus');
    
    // Check if data exists for the campus and year
    if ($is_all_campus) {
        // For "All Campus", check if any data exists for any campus for the given year
        $checkQuery = "SELECT COUNT(*) FROM gpb_entries WHERE year = ?";
        $checkStmt = $pdo->prepare($checkQuery);
        $checkStmt->execute([$year]);
    } else {
        // For specific campus, check if data exists for that campus and year
    ...
    error_log("Found {$count} records for " . ($is_all_campus ? "all campuses" : "campus '{$campus}'") . " and year '{$year}'");

    if ($count === 0) {
        echo json_encode([
            'status' => 'error',
            'message' => "No data found for " . ($is_all_campus ? "any campus" : "campus '{$campus}'") . " and year '{$year}'"
        ]);
        exit();
    }
    ...
    // Adjust where clause based on All Campus or specific campus
    if ($is_all_campus) {
        $query .= "g.year = :year";
    } else {
        $query .= "g.campus = :campus AND g.year = :year";
    }
    
    // Add status filter to only show approved entries if the status field exists
    if ($statusFieldExists) {
        $query .= " AND (g.status = 'approved' OR g.status = 'Approved')";
    }
    
    $query .= " ORDER BY " . ($is_all_campus ? "g.campus, " : "") . "g.id";

    error_log("Executing query: " . $query);
    error_log("With parameters - " . ($is_all_campus ? "" : "campus: {$campus}, ") . "year: {$year}");
    ...
    // Bind parameters based on whether we're querying for all campuses or a specific one
    if (!$is_all_campus) {
