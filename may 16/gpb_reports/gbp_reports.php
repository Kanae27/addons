...
<head>
    ...
    <!-- Global Styles included in-page -->
...
<body>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Automatically load report when year is selected
            $('#year').on('change', function() {
                const preparedBySelect = $('#preparedBy');
                
                if ($(this).val()) {
                    // Enable prepared by dropdown
            ...
            // Add event listener for preparedBy dropdown
            $('#preparedBy').on('change', function() {
                // Reload report when prepared by position changes
                if ($('#campus').val() && $('#year').val()) {
                    loadReport();
                }
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 'success' && response.data) {
                            ...
                            // Add "All Campus" option for Central users
                            campusSelect.append('<option value="All Campus">All Campus</option>');
        ...
        function loadYearOptions() {
            ...
            $.ajax({
                ...
                }, // This will work for both specific campus and "All Campus"
                dataType: 'json',
                success: function(response) {
                    ...
                    if (response.status === 'success' && response.data && response.data.length > 0) {
                        ...
                        // Add change event handler to enable prepared by dropdown
                        yearSelect.off('change').on('change', function() {
                            ...
                            if (selectedYear) {
                                preparedBySelect.prop('disabled', false);
                                // Auto-generate report when year is selected
                                loadReport();
                            } else {
                                preparedBySelect.prop('disabled', true);
                            }
                        ...
                        // If there's only one year available, select it automatically
                        if (years.length === 1) {
                            yearSelect.val(years[0].year).trigger('change');
                        }
                    } else {
                        ...
                        Swal.fire({
                            ...
                            text: `No report data is available for ${selectedCampus === 'All Campus' ? 'any campus' : selectedCampus + ' campus'}. Please select a different ${selectedCampus === 'All Campus' ? 'option' : 'campus'} or contact the administrator to add data.`
                        });
                    }
                },
                error: function(xhr, status, error) {
        ...
        function loadBudgetSummary() {
            ...
            // For "All Campus" we need to handle budget data differently
            if (selectedCampus === 'All Campus') {
                // Either hide the budget card or show a special version for "All Campus"
                $('#budgetSummaryCard').hide();
                // Create default budget data for "All Campus" view
        ...
        async function loadReport() {
            ...
            if (selectedCampus === 'All Campus') {
                // For All Campus, create a default consolidated budget object
                let budgetData = {
                    total_gaa: 0,
                    total_gad_fund: 0
                };
                
                // Skip budget summary call and proceed directly to report data
                loadReportData(selectedCampus, selectedYear, budgetData);
                return;
            }
            
            // For specific campus, load actual budget data
            $.ajax({
                url: 'api/get_budget_summary.php',
                method: 'GET',
                data: {
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success' && response.data) {
                        ...
                        if (response.data.length === 0) {
                            // Show alert that no data exists
                            Swal.fire({
                                icon: 'warning',
                                title: 'No Entries',
                                text: 'There are no entries for this campus and year.',
                            ...
                            $('#reportPreview').html(`
                                <div class="text-center text-warning py-5">
                                    ...
                                    <p>No entries found for report generation.</p>
                                </div>
                            `);
                            return;
                        }
                        ...
                        displayReport(response.data, budgetData, signatories, selectedCampus, <?php echo $isCentral ? 'true' : 'false' ?>);
                    } else {
        ...
        function displayReport(data, budgetData, signatories, selectedCampus, isCentral) {
                ...
                <div class="mb-1">
                    <div class="row">
                        <div class="col-12" style="line-height: 1.1;">
                            <p class="mb-0" style="font-size: 10px;"><strong>Campus:</strong> ${selectedCampus === 'All Campus' ? 'All Campuses' : (data[0]?.campus || '')}</p>
                    ...
                    <table class="table table-bordered" id="reportTable" style="font-size: 10px;">
                        <thead>
                            <tr style="background-color: #f8f9fa;">
                                ...
                                ${isCentral && selectedCampus === 'All Campus' ? '<th style="text-align: center; background-color: #e9ecef; font-size: 10px; width: 8%; padding: 2px;">Campus</th>' : ''}
                            ...
                            <tr style="background-color: #f8f9fa;">
                                ...
                                ${isCentral && selectedCampus === 'All Campus' ? '<th style="text-align: center; background-color: #e9ecef; font-size: 10px; padding: 2px;">(2)</th>' : ''}
                                <th style="text-align: center; background-color: #e9ecef; font-size: 10px; padding: 2px;">${isCentral && selectedCampus === 'All Campus' ? '(3)' : '(2)'}</th>
                                <th style="text-align: center; background-color: #e9ecef; font-size: 10px; padding: 2px;">${isCentral && selectedCampus === 'All Campus' ? '(4)' : '(3)'}</th>
                                <th style="text-align: center; background-color: #e9ecef; font-size: 10px; padding: 2px;">${isCentral && selectedCampus === 'All Campus' ? '(5)' : '(4)'}</th>
                                <th style="text-align: center; background-color: #e9ecef; font-size: 10px; padding: 2px; border-right: 1px solid #000;">${isCentral && selectedCampus === 'All Campus' ? '(6)' : '(5)'}</th>
                                <th style="text-align: center; background-color: #e9ecef; font-size: 10px; padding: 2px; border-left: 1px solid #000;">${isCentral && selectedCampus === 'All Campus' ? '(7)' : '(6)'}</th>
                                <th style="text-align: center; background-color: #e9ecef; font-size: 10px; padding: 2px;">${isCentral && selectedCampus === 'All Campus' ? '(8)' : '(7)'}</th>
                                <th style="text-align: center; background-color: #e9ecef; font-size: 10px; padding: 2px;">${isCentral && selectedCampus === 'All Campus' ? '(9)' : '(8)'}</th>
                                <th style="text-align: center; background-color: #e9ecef; font-size: 10px; padding: 2px;">${isCentral && selectedCampus === 'All Campus' ? '(10)' : '(9)'}</th>
                            </tr>
                        </thead>
                        <tbody>
            `;
                    ...
                    if (categoryData.length === 0) {
                        ...
                        html += `
                            <tr>
                                ...
                                ${isCentral && selectedCampus === 'All Campus' ? '<td style="font-size: 10px; padding: 2px;">&nbsp;</td>' : ''}
                                <td style="font-size: 10px; padding: 2px;">&nbsp;</td>
                                <td style="font-size: 10px; padding: 2px;">&nbsp;</td>
                                <td style="font-size: 10px; padding: 2px;">&nbsp;</td>
                        ...
                        categoryData.forEach(item => {
                            ...
                            // Format source_of_budget to remove brackets
                            let formattedSourceOfBudget = '';
                            if (item.source_of_budget) {
                                // Try to parse as JSON if it has brackets
                                if (item.source_of_budget.includes('[') || item.source_of_budget.includes(']')) {
                                    try {
                                        const sourceArray = JSON.parse(item.source_of_budget.replace(/[\[\]"]/g, (match) => {
                                            return match === '[' ? '[' : match === ']' ? ']' : '"';
                                        }));
                                        formattedSourceOfBudget = sourceArray.join(', ');
                                    } catch (e) {
                                        // If parsing fails, use as is but remove brackets
                                        formattedSourceOfBudget = item.source_of_budget.replace(/[\[\]"]/g, '');
                                    }
                                } else {
                                    formattedSourceOfBudget = item.source_of_budget;
                                }
                            }

                            // Format responsible_unit to remove brackets
                            let formattedResponsibleUnit = '';
                            if (item.responsible_unit) {
                                // Try to parse as JSON if it has brackets
                                if (item.responsible_unit.includes('[') || item.responsible_unit.includes(']')) {
                                    try {
                                        const unitArray = JSON.parse(item.responsible_unit.replace(/[\[\]"]/g, (match) => {
                                            return match === '[' ? '[' : match === ']' ? ']' : '"';
                                        }));
                                        formattedResponsibleUnit = unitArray.join(', ');
                                    } catch (e) {
                                        // If parsing fails, use as is but remove brackets
                                        formattedResponsibleUnit = item.responsible_unit.replace(/[\[\]"]/g, '');
                                    }
                                } else {
                                    formattedResponsibleUnit = item.responsible_unit;
                                }
                            }
                            ...
                            html += `
                                <tr>
                                    ...
                                    ${isCentral && selectedCampus === 'All Campus' ? `<td style="font-size: 10px; padding: 2px;">${item.campus || ''}</td>` : ''}
            ...
            // Add the signature footer at the end of the report
            html += `</tbody></table></div>
            <div style="page-break-inside: avoid !important; break-inside: avoid !important; margin-top: 15px !important;">
                <table class="table table-bordered report-footer" style="border-collapse: collapse; width: 100%; page-break-inside: avoid !important; break-inside: avoid !important;">
            ...
            try {
                ...
                if (result.status === 'success' && result.data && result.data.length > 0) {
                    ...
                    signatories = [];
                     ...
                     const selectedPosition = $('#preparedBy').val() || 'GAD Head Secretariat';
                     
                     // Function to get name for a position
                    ...
                    // Get selected position for prepared by
                    const selectedPosition = $('#preparedBy').val() || 'GAD Head Secretariat';
                    signatories = [
                        {
                            name: 'RICHELLE M. SULIT',
