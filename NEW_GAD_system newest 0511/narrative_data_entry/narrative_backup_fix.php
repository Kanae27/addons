...
.non-interactive {
    ...
    cursor: not-allowed;
    opacity: 1;
    color: #6c757d; /* Add grayed out text color */
}
...
.form-select.non-interactive:active {
    ...
    cursor: not-allowed !important;
    opacity: 1 !important;
    color: #6c757d !important; /* Add grayed out text color for focus state */
}
...
.checkbox-container, .sdgs-container {
    display: grid;
    gap: 12px;
    margin-bottom: 15px;
}
...
.checkbox-item .form-check-input {
    margin-top: 0; /* Remove top margin to help with centering */
    flex-shrink: 0;
    position: relative;
...
.checkbox-item .form-check-label {
    ...
    min-height: 24px; /* Ensure minimum height for alignment */
}
...
textarea.form-control {
    ...
    min-height: 100px;
        ...
        .narrative-table-wrapper {
            ...
            margin-bottom: 0;
            height: calc(100% - 85px); /* Reduced subtraction to show 5th row */
            min-height: 400px; /* Set reasonable min-height */
        ...
        .narrative-table.campus-user td:nth-child(1) {
            width: 50%; /* Activity column takes half the width */
        }
        
        .narrative-table.campus-user th:nth-child(2),
        .narrative-table.campus-user td:nth-child(2) {
            width: 50%; /* Partner Agency column takes half the width */
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Validate inputs
    if (!year || !quarter) {
        console.error('Missing required parameters:', { year, quarter });
        const titleSelect = document.getElementById('title');
        if (titleSelect) {
    ...
    formData.append('narrative_id', narrativeId);
                ...
                console.log('Fetching activities with:', {year, quarter});
                ...
                if (invalidSections > 0) {
                    ...
                    if (projectLeadersContainer) {
                        ...
                        rows.forEach((row, index) => {
                            ...
                            if (task) {
                                teamTasks.projectLeader.push({ name, task });
                            }
                        });
                    }
                    ...
                    if (assistantLeadersContainer) {
                        ...
                        rows.forEach((row, index) => {
                            ...
                            if (task) {
                                teamTasks.assistantLeader.push({ name, task });
                            }
                        });
                    }
                    ...
                    if (projectStaffContainer) {
                        ...
                        rows.forEach((row, index) => {
                            ...
                            if (task) {
                                teamTasks.projectStaff.push({ name, task });
                            }
                        });
                    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    containers.forEach(container => {
        if (container) {
            teamSectionObserver.observe(container, { childList: true, subtree: true });
        }
    });
...
<style>
    ...
    .narrative-modal {
        width: 90%;
        max-width: 1200px;
        height: 80vh; /* Decreased from 90vh */
        max-height: 750px; /* Decreased from 900px */
    ...
    .narrative-modal-body {
        padding: 0.75rem 1.5rem 0; /* Reduced top padding and removed bottom padding */
        overflow-y: auto;
        flex: 1;
        min-height: 300px;
        /* Hide scrollbar but maintain scroll functionality */
    ...
    .narrative-table-wrapper {
        ...
        border: 1px solid var(--border-color, #dee2e6);
        margin-bottom: 0;
        height: calc(100% - 120px); /* Further reduced subtraction */
        min-height: 300px;
        /* Hide scrollbar but maintain scroll functionality */
        scrollbar-width: none; /* Firefox */
        -ms-overflow-style: none; /* IE and Edge */
...
window.fetchActivities = function(year, quarter, narrativeId = 0) {
    ...
    function addExistingImagePreview(imageUrl, imageName) {
        ...
        const previewDiv = document.createElement('div');
        previewDiv.className = 'image-preview col-md-4 mb-3';
