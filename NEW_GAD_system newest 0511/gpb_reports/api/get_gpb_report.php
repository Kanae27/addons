...
try {
    ...
    $query .= "t.total_gaa,
        ...
        (SELECT COALESCE(SUM(p.total_male), 0) FROM ppas_forms p WHERE p.gender_issue_id = g.id AND p.campus = g.campus AND p.year = g.year) as actual_male,
        (SELECT COALESCE(SUM(p.total_female), 0) FROM ppas_forms p WHERE p.gender_issue_id = g.id AND p.campus = g.campus AND p.year = g.year) as actual_female,
        (SELECT COALESCE(SUM(p.total_beneficiaries), 0) FROM ppas_forms p WHERE p.gender_issue_id = g.id AND p.campus = g.campus AND p.year = g.year) as actual_participants,
    ...
    if (count($results) > 0) {
        ...
        // Debug with explicit logging - uncomment if you need to debug
        error_log("Gender issue IDs for activity lookup: " . implode(", ", $genderIssueIds));
        
        $activityQuery = "SELECT p.gender_issue_id, p.activity, p.total_male, p.total_female, p.ps_attribution, 
        ...
        if (!$activityStmt) {
            ...
            // Add gender issue IDs as parameters
